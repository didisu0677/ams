...
class Sub_aktivitas extends BE_Controller {
	...
	function template() {
		...
		$arr = ['sub_aktivitas' => 'sub_aktivitas','is_active' => 'is_active'];
		$config[] = [
			'title' => 'template_import_sub_aktivitas',
			'header' => $arr,
		];
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['sub_aktivitas' => 'Sub Aktivitas','is_active' => 'Aktif'];
		$data = get_data('tbl_sub_aktivitas')->result_array();
		$config = [
			'title' => 'data_sub_aktivitas',
			'data' => $data,
			'header' => $arr,
		];
		$this->load->library('simpleexcel',$config);
