...
class Daftar_aset extends BE_Controller {
	...
	function index() {
		...
		$data['opt_kota'] = get_data('tbl_m_kota',[
			'where' => [
				'is_active' => 1
			],
			'sort_by' => 'nama_kota',
		...
		$uplfile 		= get_data('tbl_upl_foto_gedung')->result();
		$data['file_foto']	= [];
	...
	function save() {
        ...
        $kota = get_data('tbl_m_kota','id',$data['id_kota'])->row();
        if(isset($kota->nama_kota)) {
		...
		if($filename != '[]' && $filename !='' && count($filename) > 0){
			$data['propar'] = 1;
		}else{
			$data['propar'] = 0;
		}
		...
		if($response['status'] == 'success') {
			...
			$old_file_foto 			= post('old_file_foto');

			// debug($old_file_foto );die;
			...
			foreach($id_foto as $k => $v) {
				...
				$d[$k] 			= [
					...
					'file_foto'			=> $old_file_foto[$k],
				];
				if($file_foto[$k] && $file_foto[$k] != $old_file_foto[$k]) {
					if(@copy($file_foto[$k], FCPATH . 'assets/uploads/foto_gedung/'.$response['id'].'/'.basename($file_foto[$k]))) {
						...
						if($old_file_foto[$k]) {
							@unlink(FCPATH . 'assets/uploads/foto_gedung/'.$response['id'].'/'.$old_file_foto[$k]);
						}
					}
				}elseif(!$file_foto[$k]) {
				    $d[$k]['file_foto'] = '';
			...
			delete_data('tbl_upl_foto_gedung','id_asset',$id_asset->id);
			if(count($d)) {
	...
	function delete() {
		...
		$response = destroy_data('tbl_upl_foto_gedung','id_asset',post('id'));
	...
	function template() {
		...
		$arr = ['nomor' => 'ID Asset','nama_gedung' => 'nama_gedung','alamat' => 'alamat','kota' => 'kota','titik_koordinat' => 'titik_koordincat','pemilik' => 'pemilik','mitra' => 'mitra','facility_management' => 'facility_management','status_asset' => 'status_asset','skema_kerjasama' => 'skema_kerjasama','status_tanah' => 'status tanah','masa_berlaku_surat_tanah' => 'masa_berlaku_surat_tanah','luas_tanah' => 'luas_tanah','LXP_tanah' => 'lxp_tanah','bentuk_tanah' => 'bentuk_tanah','luas_bangunan' => 'luas_bangunan','jumlah_ltbangunan' => 'jumlah_ltbangunan','luas_area_idle' => 'luas_area_idle','jenis_bangunan' => 'jenis_bangunan','tipe_asset' => 'tipe_asset','batasan_lahan' => 'batasan_lahan','informasi_sekitar' => 'informasi_sekitar','status_penggunaan' => 'status_penggunaan','usulan_mitra' => 'usulan_mitra','catu_daya' => 'catu_daya','area' => 'area','keterangan' => 'keterangan'];
		$config[] = [
	...
	function import() {
		...
		$col = ['nomor','nama_gedung','alamat','kota','titik_koordinat','pemilik','mitra','facility_management','status_asset','skema_kerjasama','status_tanah','masa_berlaku_surat_tanah','luas_tanah','LXP_tanah','bentuk_tanah','luas_bangunan','jumlah_ltbangunan','luas_area_idle','jenis_bangunan','batasan_lahan','informasi_sekitar','status_penggunaan','usulan_mitra','catu_daya','area','keterangan'];
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['nomor' => 'ID Asset','nama_gedung' => 'Nama Gedung','alamat' => 'Alamat','kota' => 'Kota','titik_koordinat' => 'Titik Koordinat','pemilik' => 'Pemilik','status_asset' => 'Status Asset','perusahaan'=>'Mitra','skema_kerjasama' => 'Skema Kerjasama','status_tanah' => 'Status Surat Tanah','masa_berlaku_surat_tanah' =>'Masa Berlaku Surat Tanah','luas_tanah' => 'Luas Tanah','LXP_tanah' => 'Panjang x Lebar Tanah','bentuk_tanah' => 'Bentuk Tanah','luas_bangunan' => 'Luas Bangunan','jumlah_ltbangunan' => 'Jumlah Lantai dan Bangunan','luas_area_idle' => 'Luas Area Idle','jenis_bangunan' => 'Jenis Asset','tipe_asset'=>'Tipe Asset','batasan_lahan' => 'Batasan Lahan','informasi_sekitar' => 'Informasi Sekitar','status_penggunaan' => 'Pengguna saat ini','usulan_penggunaan' => 'Usulan Penggunaan','usulan_mitra' => 'Usulan Mitra','jarak_usulan_mitra_lokasi'=>'Jarak usulan mitra ke lokasi','catu_daya' => 'Catu Daya','area' => 'Area','facility_management'=>'Facility Management (FM)','update_terakhir'=>'Update terakhir','keterangan' => 'Keterangan'];
		$data = get_data('tbl_asset a',[
				'select' => 'a.*,b.pemilik_aset as pemilik, c.jenis_bangunan,d.skema_kerjasama,e.area,f.perusahaan,g.tipe_asset,
				CASE
				...
				'join' 	 => ['tbl_pemilik_aset b on a.id_pemilik =b.id type LEFT',
							 ...
							 'tbl_m_mitra f on a.id_mitra = f.id type LEFT',
							 'tbl_m_tipe_asset g on a.id_tipe_asset = g.id type LEFT',
				],
		])->result_array();
		$config = [
