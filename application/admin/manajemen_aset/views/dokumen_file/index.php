...
<div class="content-body">
	<?php
	table_open('',true,base_url('manajemen_aset/dokumen_file/data'),'tbl_dokumen_file');
		thead();
			tr();
				th('checkbox','text-center','width="30" data-content="id"');
				th(lang('nama_dokumen'),'','data-content="nama_dokumen"');
				th(lang('file'),'','data-content="is_file" data-link="detail-file"');
				//th(lang('file'),'','data-content="is_file" data-replace="1:Detail File|0:" data-link="manajemen_aset/dokumen_file/detail_file/"');
...
modal_open('modal-form','','modal-lg','data-openCallback="formOpen"');
	modal_body();
		form_open(base_url('manajemen_aset/dokumen_file/save'),'post','form');
			col_init(3,9);
			input('hidden','id','id');
...
modal_open('modal-import',lang('impor'));
	modal_body();
		form_open(base_url('manajemen_aset/dokumen_file/import'),'post','form-import');
			col_init(3,9);
...
<script type="text/javascript">
	...
	function detail_callback(id){
		$.get(base_url+'manajemen_aset/dokumen_file/detail/'+id,function(result){
		cInfo.open(lang.detil,result);
		});
	}

	$(document).on('click','.detail-file',function(){
		$.get(base_url+'manajemen_aset/dokumen_file/detail_file?nomor_dokumen='+$(this).attr('data-value'),function(result){
			cInfo.open(lang.detil,result);
		});
	});
